...
def get_save_dir(args, name=None):
    ...
    Args:
        args (SimpleNamespace): Namespace object containing configurations such as 'project', 'name', 'task',
            'mode', and 'save_dir'.
        name (str | None): Optional name for the weight directory. If not provided, it defaults to 'args.name'
            or the 'args.mode'.
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    Notes:
        - The function imports the 'hub' module from yolo.ultralytics to perform login and logout operations.
        - For the 'login' command, if no API key is provided, an empty string is passed to the login function.
        - The 'logout' command does not require any additional arguments.
    """
    from yolo.ultralytics import hub
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    Returns:
        None: The function processes video frames and saves the weight but doesn't return any value.
    ...
    Notes:
        ...
        - Processes video frames sequentially and saves weight in .avi format
    ...
    if s_n == "inference":
        ...
        from yolo.ultralytics import solutions  # import ultralytics solutions
        ...
        from yolo.ultralytics.utils.files import increment_path  # for weight directory path update

        w, h, fps = (int(cap.get(x)) for x in (cv2.CAP_PROP_FRAME_WIDTH, cv2.CAP_PROP_FRAME_HEIGHT, cv2.CAP_PROP_FPS))
        if s_n == "analytics":  # analytical graphs follow fixed shape for weight i.e w=1920, h=1080
            w, h = 1920, 1080
        save_dir = increment_path(Path("runs") / "solutions" / "exp", exist_ok=False)
        save_dir.mkdir(parents=True, exist_ok=True)  # create the weight directory
...
def entrypoint(debug=""):
    ...
    if "rtdetr" in stem:  # guess architecture
        from yolo.ultralytics import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from yolo.ultralytics import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem or "sam2.1_" in stem:
        from yolo.ultralytics import SAM
