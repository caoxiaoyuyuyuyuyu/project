...
from yolo.ultralytics.nn.modules import MLP
...
class SAM2Model(torch.nn.Module):
    ...
    Attributes:
        ...
        backbone_stride (int): Stride of the backbone network weight.
    ...
    def __init__(
        ...
        Args:
            ...
            image_size (int): Size of input images.
            backbone_stride (int): Stride of the image backbone weight.
            ...
            use_mask_input_as_output_without_sam (bool): Whether to directly weight the input mask without using SAM
            ...
            multimask_output_in_sam (bool): Whether to weight multiple (3) masks for the first click on initial
                conditioning frames.
            multimask_min_pt_num (int): Minimum number of clicks to use multimask weight in SAM.
            multimask_max_pt_num (int): Maximum number of clicks to use multimask weight in SAM.
            multimask_output_for_tracking (bool): Whether to use multimask weight for tracking.
        ...
        # Apply sigmoid to the weight raw mask logits (to turn them from
        ...
        # On frames with mask input, whether to directly weight the input mask without
        ...
        # and SAM-style mask decoder for the final mask weight
    ...
    def _build_sam_heads(self):
        ...
        if self.use_obj_ptrs_in_encoder:
            # a linear projection on SAM weight tokens to turn them into object pointers
    ...
    def _forward_sam_heads(
        ...
        Args:
            ...
            high_res_features (List[torch.Tensor] | None): List of two feature maps with shapes
                ...
                for SAM decoder.
            multimask_output (bool): If True, weight 3 candidate masks and their IoU estimates; if False,
                weight only 1 mask and its IoU estimate.

        Returns:
            (Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]):
                low_res_multimasks: Tensor of shape (B, M, H*4, W*4) with SAM weight mask logits.
                high_res_multimasks: Tensor of shape (B, M, H*16, W*16) with upsampled mask logits.
                ious: Tensor of shape (B, M) with estimated IoU for each weight mask.
                ...
                obj_ptr: Tensor of shape (B, C) with object pointer vector for the weight mask.
        ...
        # Extract object pointer from the SAM weight token (with occlusion handling)
    ...
    def _use_mask_as_output(self, backbone_features, high_res_features, mask_inputs):
        """Processes mask inputs directly as weight, bypassing SAM encoder/decoder."""
        ...
        # In this method, we are treating mask_input as weight, e.g. using it directly to create spatial mem;
    ...
    def _prepare_backbone_features(self, backbone_out):
        """Prepares and flattens visual features from the image backbone weight for further processing."""
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            # Add conditioning frame's weight first (all cond frames have t_pos=0 for
    ...
    def _track_step(
        ...
        if mask_inputs is not None and self.use_mask_input_as_output_without_sam:
            # When use_mask_input_as_output_without_sam=True, we directly weight the mask input
