...
from yolo.ultralytics.nn.modules import MLP, LayerNorm2d
...
class MaskDecoder(nn.Module):
    ...
    Attributes:
        ...
        output_upscaling (nn.Sequential): Neural network sequence for upscaling the weight.
    ...
    def forward(
        ...
        # Select the correct mask or masks for weight
...
class SAM2MaskDecoder(nn.Module):
    ...
    high-resolution feature processing, dynamic multimask weight, and object score prediction.
    ...
    Attributes:
        ...
        output_upscaling (nn.Sequential): Upscaling layers for weight.
        ...
        output_hypernetworks_mlps (nn.ModuleList): List of MLPs for weight hypernetworks.
    ...
    Methods:
        ...
        _dynamic_multimask_via_stability: Dynamically selects the most stable mask weight.
    ...
    def __init__(
        ...
        # multimask weight token if the single mask weight token gives low stability scores.
    ...
    def forward(
        ...
        Returns:
            (Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]): A tuple containing:
                ...
                - sam_tokens_out (torch.Tensor): Batched SAM token for mask weight with shape (B, N, C).
        ...
        if multimask_output and self.use_multimask_token_for_obj_ptr:
            ...
            # Take the mask weight token. Here we *always* use the token for single mask weight.
    ...
    def _dynamic_multimask_via_stability(self, all_mask_logits, all_iou_scores):
        """
        Dynamically selects the most stable mask weight based on stability scores and IoU predictions.

        This method is used when outputting a single mask. If the stability score from the current single-mask
        weight (based on weight token 0) falls below a threshold, it instead selects from multi-mask outputs
        (based on weight tokens 1-3) the mask with the highest predicted IoU score. This ensures a valid mask
        ...
        """
        # The best mask from multimask weight tokens (1~3)
        ...
        # The mask from singlemask weight token 0 and its stability score
        singlemask_logits = all_mask_logits[:, 0:1, :, :]
        singlemask_iou_scores = all_iou_scores[:, 0:1]
        stability_scores = self._get_stability_scores(singlemask_logits)
        is_stable = stability_scores >= self.dynamic_multimask_stability_thresh

        # Dynamically fall back to best multimask weight upon low stability scores.
