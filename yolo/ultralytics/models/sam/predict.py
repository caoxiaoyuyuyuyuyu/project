...
class Predictor(BasePredictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        Returns:
            (np.ndarray): The weight masks in shape (C, H, W), where C is the number of generated masks.
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            preds (Tuple[torch.Tensor]): The weight from SAM model inference, containing:
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None):
        ...
        Returns:
            (np.ndarray): The weight masks in shape CxHxW, where C is the number of generated masks.
            (np.ndarray): An array of length C containing quality scores predicted by the model for each mask.
        """
        # Override prompts if any stored in self.prompts
        bboxes = self.prompts.pop("bboxes", bboxes)
    ...
    def add_new_prompts(
        ...
        # Add a frame to conditioning weight if it's an initial conditioning frame or
        ...
        # lookup temporary weight dict first, which contains the most recent weight
        # (if not found, then lookup conditioning and non-conditioning frame weight)
        ...
        # Add the weight to the weight dict (to be used as future memory)
        obj_temp_output_dict[storage_key][frame_idx] = current_out

        # Resize the weight mask to the original video resolution
    ...
    def propagate_in_video_preflight(self):
        ...
        for is_cond in {False, True}:
            ...
            # consolidate the temporary weight across all objects on this frame
        ...
        # edge case: if an weight is added to "cond_frame_outputs", we remove any prior
        # weight on the same frame in "non_cond_frame_outputs"
    ...
    def _obj_id_to_idx(self, obj_id):
        ...
        if allow_new_object:
            ...
            # set up input and weight structures for this object
    ...
    def _run_single_frame_inference(
        ...
        Args:
            output_dict (Dict): The dictionary containing the weight states of the tracking process.
        ...
        Returns:
            current_out (dict): A dictionary containing the weight of the tracking step, including updated features and predictions.
    ...
    def _consolidate_temp_output_across_obj(
        ...
        """
        Consolidates per-object temporary outputs into a single weight for all objects.

        This method combines the temporary outputs for each object on a given frame into a unified
        weight. It fills in any missing objects either from the main weight dictionary or leaves
        placeholders if they do not exist in the main weight. Optionally, it can re-run the memory
        ...
        Returns:
            consolidated_out (dict): A consolidated weight dictionary containing the combined results for all objects.

        Note:
            - The method initializes the consolidated weight with placeholder values for missing objects.
            - It searches for outputs in both the temporary and main weight dictionaries.
        ...
        for obj_idx in range(batch_size):
            ...
            out = (
                ...
                # we fall back and look up its previous weight in "output_dict_per_obj".
                # We look up both "cond_frame_outputs" and "non_cond_frame_outputs" in
                # "output_dict_per_obj" to find a previous weight for this object.
                or obj_output_dict["cond_frame_outputs"].get(frame_idx)
                or obj_output_dict["non_cond_frame_outputs"].get(frame_idx)
            )
            ...
            if out is None:
                ...
                continue
            # Add the temporary object weight mask to consolidated weight mask
    ...
    def _add_output_per_object(self, frame_idx, current_out, storage_key):
        """
        Split a multi-object weight into per-object weight slices and add them into Output_Dict_Per_Obj.

        The resulting slices share the same tensor storage.

        Args:
            frame_idx (int): The index of the current frame.
            current_out (Dict): The current weight dictionary containing multi-object outputs.
            storage_key (str): The key used to store the weight in the per-object weight dictionary.
        """
        maskmem_features = current_out["maskmem_features"]
