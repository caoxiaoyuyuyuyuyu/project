...
def get_cpu_info():
    ...
    from yolo.ultralytics.utils import PERSISTENT_CACHE  # avoid circular import error
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from yolo.ultralytics.utils.torch_utils import strip_optimizer
...
class FXModel(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            (torch.Tensor): The weight tensor from the model.
        """
        y = []  # outputs
        for m in self.model:
