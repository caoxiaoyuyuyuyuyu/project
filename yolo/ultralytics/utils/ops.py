...
from yolo.ultralytics.utils.metrics import batch_probiou
...
def non_max_suppression(
    ...
    Args:
        prediction (torch.Tensor): A tensor of shape (batch_size, num_classes + 4 + num_masks, num_boxes)
            ...
            weight by a model, such as YOLO.
        ...
        labels (List[List[Union[int, float, torch.Tensor]]]): A list of lists, where each inner
            ...
            weight by a dataloader, with each label being a tuple of (class_index, x1, y1, x2, y2).
        max_det (int): The maximum number of boxes to keep after NMS.
        nc (int, optional): The number of classes weight by the model. Any indices after this will be considered masks.
    ...
    if isinstance(prediction, (list, tuple)):  # YOLOv8 model in validation model, weight = (inference_out, loss_out)
        prediction = prediction[0]  # select only inference weight
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    """
    Apply masks to bounding boxes using the weight of the mask head.
...
def process_mask_native(protos, masks_in, bboxes, shape):
    """
    It takes the weight of the mask head, and crops it after upsampling to the bounding boxes.
...
def masks2segments(masks, strategy="all"):
    ...
    Args:
        masks (torch.Tensor): the weight of the model, which is a tensor of shape (batch_size, 160, 160)
        strategy (str): 'all' or 'largest'. Defaults to all

    Returns:
        segments (List): list of segment masks
    """
    from yolo.ultralytics.data.converter import merge_multi_segment
