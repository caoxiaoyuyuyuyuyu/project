...
from ocr.generatorMQ import GeneratorMQ
...
def train(args):
    ...
    # log
    writer = SummaryWriter(log_dir='log') if utils.is_main_process() else None
    ...
    for epoch in range(args.init_epoch, args.epochs):
        ...
        if utils.is_main_process():
            model.eval()
            predicted_label = inference_image(model, '0123456789+-x÷=', args.test_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight-dir', default='./weight', help='path where to save')
    parser.add_argument('--test-img', type=str, default='test.jpg', help='path of test image to be predicted')
